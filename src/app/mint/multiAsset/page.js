...
const WriteView = () => {
    ...
    useEffect(() => {
        ...
        if (typeof window !== undefined) {
            const helper = async () => {
                ...
                settokenId(
                    parseInt(((await contract.tokenCount()) - 1).toString())
                );
            };
            helper();
        }
    ...
    const handleMint = async () => {
        ...
        try {
            const txResponse = await contract.mint(ipfsURI, false);
            ...
            fileUris.map(async (file, i) => {
                await (
                    await contract.addAssets(parseInt(tokenId) + 1, file)
                ).wait();
            });
            setMinting(false);
