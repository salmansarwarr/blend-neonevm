...
import { getByAddress, getByAddressEquippable } from "@/utils/api";
...
const page = () => {
    ...
    useEffect(() => {
        const helperFunc = async () => {
            ...
            const { data } = await getByAddressEquippable(address);
            const filteredData = data.filter(
                (item) => item.type === "ERC-6220"
            );
            ...
            {nfts.length == 0 ? (
                ...
                <div className="bg-white w-full min-h-screen text-black flex flex-col items-center gap-6 py-20">
                    ...
                    <div className="flex flex-col items-center gap-3">
                        {nfts.map((nft) => (
                            <Link
                                href={`/your-nfts/equippable/${nft.tokenId}`}
                                key={nft._id}
