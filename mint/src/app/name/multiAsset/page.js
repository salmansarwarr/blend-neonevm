...
import { sendFileToPinata, storeTokenUriMetadata } from "@/utils/pinata";
import Loader from "@/components/Loader";
import { toast, ToastContainer } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { useRouter, useSearchParams } from "next/navigation";
import { getObjFromIndexedDB } from "@/utils/functions";
...
const Name = () => {
    ...
    useEffect(() => {
        if ((!isPaid || isPaid == "false") && localStorageChecked) {
            ...
            router.push("/payment/multiAsset");
        }
    });
    ...
    const handlePinataUri = async () => {
        const { data: meshFile } = await encrypt(obj);
        const metadata = {
            name,
            obj: meshFile,
            thumbnail:
                "https://red-ready-marlin-412.mypinata.cloud/ipfs/QmNXTQ2CNmjn7o4vJK1V7Ch53zaHShaHci84NshuWaZhYt?_gl=1*1mbixbc*_ga*MTk0Mzk3OTkyNy4xNzAyMDYzMjky*_ga_5RMPXG14TE*MTcwMzE2NTI4MC45LjEuMTcwMzE2NTMyMy4xNy4wLjA.",
        };

        let IpfsUris = [];
        const files = await getObjFromIndexedDB();

        for (let i = 1; i < files.length; i++) {
            const file = files[i];
            try {
                const res = await sendFileToPinata(file);
                IpfsUris.push(
        ...
        <>
            ...
            >
                ...
                <div className="flex w-full py-24 gap-8 flex-col items-center">
                    ...
                    {loading ? (
                        <div
                            className={`flex  items-center justify-centerz-50`}
                        >
                            <div role="status">
                                <svg
                                    aria-hidden="true"
                        ...
                        <div
                            ...
                            className="flex items-center px-8 py-3 border-[4px]"
                        >
                            <input
                                type="text"
